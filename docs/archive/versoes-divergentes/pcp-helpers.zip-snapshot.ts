...
export async function buscarOSMubiSys(osNumero: string): Promise<MubiSysOS | null> {
  try {
    ...
    if (!token || !publicKey) {
      console.error("[PCP] Credenciais MubiSys não configuradas");
      return null;
    }

    const response = await fetch(`https://api.mubisys.com/api/${publicKey}/ordem-servico`, {
      headers: {
        "Access-Token": token,
      },
    });
    ...
    if (!response.ok) {
      console.error("[PCP] Erro ao buscar OS no MubiSys:", response.statusText);
      return null;
    }

    const data = await response.json();
    const ordens = Array.isArray(data) ? data : data.data || [];

    // Procurar pela OS específica
    const os = ordens.find((o: any) => o.numero === osNumero || o.id === osNumero);
    return os || null;
  } catch (error) {
...
export async function listarOrdensProducao(filtro?: { status?: string; cliente?: string }): Promise<ProducaoOrdem[]> {
  const db = await getDb();
  try {
    let query = db.select().from(producaoOrdens);

    if (filtro?.status) {
      query = query.where(eq(producaoOrdens.statusGeral, filtro.status as any));
    }

    if (filtro?.cliente) {
      query = query.where(eq(producaoOrdens.clienteNome, filtro.cliente));
    }

    return await query.orderBy(desc(producaoOrdens.criadoEm));
  } catch (error) {
...
export async function deletarOrdemProducao(ordemId: number): Promise<boolean> {
  ...
  try {
    ...
    await db.delete(producaoHistoricoAlteracoes).where(eq(producaoHistoricoAlteracoes.ordemId, ordemId));
