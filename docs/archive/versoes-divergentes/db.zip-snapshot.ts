...
export async function createLocalUser(data: InsertLocalUser) {
  const db = await getDb();
  if (!db) throw new Error("DB not available");
  return db.insert(localUsers).values(data);
}
...
export async function upsertFinanceiro(input: {
  ...
  if (existing) {
    await db.update(financeirosMensais).set({
      receitaBruta: input.receitaBruta ?? existing.receitaBruta,
      receitaOperacional: input.receitaOperacional ?? existing.receitaOperacional,
      receitaFinanceira: input.receitaFinanceira ?? existing.receitaFinanceira,
      despesasTotal: input.despesasTotal ?? existing.despesasTotal,
      despesasFixas: input.despesasFixas ?? existing.despesasFixas,
      despesasVariaveis: input.despesasVariaveis ?? existing.despesasVariaveis,
      despesasPessoal: input.despesasPessoal ?? existing.despesasPessoal,
      despesasFinanceiras: input.despesasFinanceiras ?? existing.despesasFinanceiras,
      despesasImpostos: input.despesasImpostos ?? existing.despesasImpostos,
      lucroGruto: input.lucroGruto ?? existing.lucroGruto,
      lucroOperacional: input.lucroOperacional ?? existing.lucroOperacional,
      lucroLiquido: input.lucroLiquido ?? existing.lucroLiquido,
      entradas: input.entradas ?? existing.entradas,
      saidas: input.saidas ?? existing.saidas,
      saldoMes: input.saldoMes ?? existing.saldoMes,
    ...
    const result = await db.insert(financeirosMensais).values({
      mes: input.mes,
      ano: input.ano,
      receitaBruta: input.receitaBruta ?? 0,
      receitaOperacional: input.receitaOperacional ?? 0,
      receitaFinanceira: input.receitaFinanceira ?? 0,
      despesasTotal: input.despesasTotal ?? 0,
      despesasFixas: input.despesasFixas ?? 0,
      despesasVariaveis: input.despesasVariaveis ?? 0,
      despesasPessoal: input.despesasPessoal ?? 0,
      despesasFinanceiras: input.despesasFinanceiras ?? 0,
      despesasImpostos: input.despesasImpostos ?? 0,
      lucroGruto: input.lucroGruto ?? 0,
      lucroOperacional: input.lucroOperacional ?? 0,
      lucroLiquido: input.lucroLiquido ?? 0,
      entradas: input.entradas ?? 0,
      saidas: input.saidas ?? 0,
      saldoMes: input.saldoMes ?? 0,
      fonte: "manual",
